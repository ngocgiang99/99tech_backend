...
export class RateLimitGuard implements CanActivate {
  ...
  constructor(
    ...
    @Inject(METRIC_RATE_LIMIT_FAILED_CLOSED_TOTAL)
    private readonly rateLimitFailedClosedTotal: Counter<string>,
  ) {}
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    let result: Awaited<ReturnType<RedisTokenBucket['consume']>>;
    try {
      result = await this.bucket.consume(userId, 20, refillPerSec);
    } catch (err) {
      this.rateLimitFailedClosedTotal.inc();
      this.logger.error(
        { err },
