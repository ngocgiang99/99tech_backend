// ---------------------------------------------------------------------------
// Mock jose before any imports — JwksCache (imported via JwtGuard) uses jose
// ---------------------------------------------------------------------------
jest.mock('jose', () => ({
...
describe('LeaderboardController unit tests', () => {
  ...
  it('empty cache → falls back to DB, sets X-Cache-Status header, maps rows to entries', async () => {
    ...
    const e = result.entries as Array<{ rank: number; userId: string; score: number }>;
    expect(e[0]).toMatchObject({ rank: 1, userId: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', score: 300 });
    expect(e[1]).toMatchObject({ rank: 2, userId: 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', score: 200 });
    expect(res.header).toHaveBeenCalledWith('X-Cache-Status', 'miss-fallback');
  });

  it('limit > 100 → BadRequestException', async () => {
    const controller = new LeaderboardController(makeCacheMock([]), makeDbMock([]));
    await expect(
  ...
  it('invalid string limit → BadRequestException', async () => {
    const controller = new LeaderboardController(makeCacheMock([]), makeDbMock([]));
    await expect(
      controller.getTop({ limit: 'notanumber' } as unknown, makeResMock() as never),
    ).rejects.toThrow(BadRequestException);
  });

  it('no limit param → defaults to 10, cache.getTop called with 10', async () => {
    const cache = makeCacheMock([makeEntry(1, 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 100)]);
  ...
  it('generatedAt is a valid ISO date string', async () => {
    const cache = makeCacheMock([makeEntry(1, 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 100)]);
    const controller = new LeaderboardController(cache, makeDbMock([]));
    const result = await controller.getTop({ limit: '1' } as unknown, makeResMock() as never);
