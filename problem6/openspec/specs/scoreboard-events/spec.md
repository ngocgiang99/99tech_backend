...
The NATS JetStream infrastructure and the outbox publisher worker for the scoreboard module. Owns the shared `NatsConnection` + JetStream context, the `StreamBootstrap` that idempotently provisions the `SCOREBOARD` stream on boot, the `JetStreamEventPublisher` implementing the `DomainEventPublisher` port, the `OutboxPublisherService` with Redis-lock leader election and 100ms coalescing window for leaderboard updates, and the `JetStreamSubscriber` ephemeral push consumer that forwards messages into a local in-process emitter. This is the messaging plumbing that turns committed Postgres outbox rows into NATS messages and back into in-process events for the SSE controller in `scoreboard-streaming`.
