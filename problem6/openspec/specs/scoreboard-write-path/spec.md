...
The `ScoreboardController.incrementScore()` and `ActionsController.issueActionToken()` methods SHALL NOT contain ANY local try/catch blocks. All errors propagate to the global exception filter (`scoreboard-observability` capability). The previously-documented `IdempotencyViolationError` replay catch in `ScoreboardController` has been MOVED into `IncrementScoreHandler` as part of the `enforce-ddd-layer-boundaries` change — the handler now owns idempotent-replay recovery and returns a tagged result rather than throwing.

#### Scenario: Controllers do not contain catch blocks
- **WHEN** `grep -E "catch \\(" src/scoreboard/interface/http/controllers/` is run
- **THEN** there are zero matches inside `scoreboard.controller.ts` and `actions.controller.ts`
- **AND** no controller method maps domain errors to HTTP codes
...
#### Scenario: IdempotencyViolationError replay is handled by the application handler, not the controller
- **GIVEN** a duplicate credit request that triggers `IdempotencyViolationError` inside `repository.credit()`
- **WHEN** `IncrementScoreHandler.execute()` runs
- **THEN** the handler catches the error internally, looks up the prior outcome via `repository.findScoreEventByActionId()`, and returns `{ kind: 'idempotent-replay', ... }`
- **AND** the controller receives the tagged result and returns 200 with the body (`kind` stripped)
- **AND** the controller has no `try/catch` block at all — the handler's contract guarantees no `IdempotencyViolationError` escapes

### Requirement: Handler writes outbox row inside the existing transaction

`IncrementScoreHandler.execute()` SHALL be modified to also insert a row into `outbox_events` inside the SAME `BEGIN…COMMIT` block as the `score_events` INSERT and `user_scores` upsert. The repository's `credit()` method SHALL accept a third parameter `outboxRow: OutboxRow` (typed as `{ aggregateId: string, eventType: string, payload: Record<string, unknown> }`) and the repository SHALL coordinate the three SQL writes inside the transaction.
...
`IncrementScoreHandler.execute()` SHALL be modified to call `leaderboardCache.upsert()`, `leaderboardCache.getRank()`, and `leaderboardCache.getTop(10)` AFTER the transaction commits. The response DTO SHALL be `{ userId, newScore, rank: number | null, topChanged: boolean | null }` where `rank` is the user's 1-indexed position (or null on error/cache-down) and `topChanged` is true if the user is now in the top 10 (or null on error). Cache failures SHALL NOT fail the request — the score is already durable in Postgres.
