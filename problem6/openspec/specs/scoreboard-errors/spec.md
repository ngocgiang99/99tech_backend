...
Shared typed-error primitives for the scoreboard module. Owns the `DomainError` hierarchy, the `ERROR_CODE_META` code-to-status table, the `buildErrorMetadata` structured log payload builder, the `scrubHeaders` redaction helper, the `mapDbError` Postgres SQLSTATE mapper, the `toPublicResponse` allowlist envelope builder, and the `wrapUnknown` coercion function used by the global HTTP exception filter. This is the "shared primitives" layer that the interface, application, and infrastructure layers all import from, living at `src/scoreboard/shared/errors/`. It is framework-adjacent (NestJS-aware via `HttpException` extension) but has no dependency on Fastify, Pino, Kysely, or ioredis — all dependencies are injected through parameters or detected structurally.
