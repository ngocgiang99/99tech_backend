...
Three-layer idempotency contract for the scoreboard module. Owns the layer-1 Redis SETNX implementation (fast path in `ActionTokenGuard`), the layer-2 catch-and-replay logic in the controller's error path (which reads the prior outcome from Postgres `score_events`), and the documentation of layer-3 (NATS JetStream `Nats-Msg-Id` dedup, wired in `step-05`/`step-06`).
