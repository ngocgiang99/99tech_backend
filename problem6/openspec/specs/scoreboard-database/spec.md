...
Typed Postgres data layer via Kysely. Owns the migration runner (`kysely-ctl`), the v1 schema (`score_events` + `user_scores`, deferring `outbox_events` to a later change), the codegen pipeline (`kysely-codegen` → `src/database/types.generated.ts`), and the `Database` factory that builds the `pg` pool from `ConfigService`. Provides a single injectable `Kysely<DB>` instance to downstream adapters.
