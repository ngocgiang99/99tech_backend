...
Defines the contract for how the resources API's performance is measured: which
load scenarios exist, what SLOs each scenario enforces, how the data is seeded,
how thresholds are wired into k6, and how results are recorded for review. The
goal is that a reviewer can read `Benchmark.md` and understand the service's
throughput, latency, error envelope, and the cache's empirical contribution
without re-running the suite — and that any reviewer who wants to re-run a
scenario can do so with a single `pnpm bench:<name>` command against the
running compose stack.

This capability is deliberately *evidence-oriented*: scenarios ship as reusable
k6 scripts under `benchmarks/` (not one-off shell invocations), thresholds
live in a shared module so the same SLO appears in every scenario, and the
seed pool is large enough that cache hit rates reflect realistic repeated
access. The cache-cold vs cache-warm comparison is the primary architectural
signal — it answers "is the Redis layer earning its complexity cost?".
