...
- [ ] 1.1 Create `src/shared/error-codes.ts` exporting `ERROR_CODES` as an `as const` tuple and the `ErrorCode` type union
...
- [ ] 1.4 Add unit test `tests/unit/shared/error-codes.test.ts` asserting every code in the tuple has a meta entry and the status codes are in the expected HTTP ranges

## 2. Expanded Error Class Hierarchy

- [ ] 2.1 Rewrite `src/shared/errors.ts` with an `AppError` base class carrying `code`, `status`, `message`, `details?`, and `cause?`
...
- [ ] 2.6 Add unit test `tests/unit/shared/errors.test.ts` asserting: every subclass extends `AppError`, every subclass sets the correct default status, `wrapUnknown(new Error("..."))` returns `InternalError`, `wrapUnknown(new ValidationError(...))` is a pass-through

## 3. Sensitive-Data Scrubber

- [ ] 3.1 Create `src/shared/sanitizer.ts` exporting `scrubHeaders(headers: Record<string, unknown>): Record<string, unknown>`
...
- [ ] 3.5 Add unit test `tests/unit/shared/sanitizer.test.ts` covering: case-insensitive matching, extra headers from env, preservation of non-denylisted values, handling of array-valued headers (e.g., multi-value Set-Cookie)

## 4. Error Metadata Builder

- [ ] 4.1 Create `src/shared/error-metadata.ts` exporting `buildErrorMetadata(err, req, res): ErrorMetadata`
...
- [ ] 4.4 Add unit test `tests/unit/shared/error-metadata.test.ts` asserting: all required fields present, body bytes never appear, cause chain is walked correctly, sensitive headers are scrubbed in the returned object

## 5. Public Response Formatter

- [ ] 5.1 Create `src/shared/to-public-response.ts` exporting `toPublicResponse(err: AppError, requestId: string, errorId: string | null): { status: number, body: unknown }`
...
- [ ] 5.5 Add unit test `tests/unit/shared/to-public-response.test.ts` covering: validation response has details, not-found response has no details and no errorId, internal error response has errorId and generic message, message truncation

## 6. Infrastructure Error Mapper

- [ ] 6.1 Create `src/infrastructure/db/error-mapper.ts` exporting `mapDbError(err: unknown): AppError`
...
- [ ] 6.4 Modify `src/modules/resources/infrastructure/repository.ts` to wrap every query in `try { ... } catch (err) { throw mapDbError(err) }`
- [ ] 6.5 Add unit test `tests/unit/infrastructure/db/error-mapper.test.ts` covering each mapped pg code plus one unknown code
...
- [ ] 8.1 Audit `src/modules/resources/presentation/controller.ts` for any `throw new Error(...)` or bare `throw`; replace with typed `AppError` subclasses
- [ ] 8.2 Audit `src/modules/resources/application/service.ts` similarly; use `NotFoundError` for missing resources, `ValidationError` for domain validation failures, `ConflictError` for state conflicts
- [ ] 8.3 Audit `src/modules/resources/infrastructure/repository.ts` — every query catches and calls `mapDbError`
...
- [ ] 9.1 Modify `src/shared/logger.ts` (or add `src/shared/log-with-metadata.ts`) to export `logWithMetadata(logger, level, err, context)` — a helper for any "I need to log a failure outside the middleware" path
