...
**List/pagination** uses keyset (cursor) pagination, not offset. The cursor is an opaque base64url string encoding a discriminated union `{ kind, value, id, sort }` — `kind: 'timestamp'` carries a `Date` value for `-createdAt`/`createdAt`/`-updatedAt`/`updatedAt` sorts, `kind: 'name'` carries a `string` value (the last row's name) for `name`/`-name` sorts. The codec and variants live in `src/modules/resources/infrastructure/cursor.ts`. The raw repository decodes the cursor internally in `list()` (it never escapes to layers above) and encodes the `nextCursor` before returning, so service/cached-repo/controller only ever see the opaque string. Predicate construction uses a generic `applyCursorPredicate<V extends Date | string>` that discriminates on `cursor.kind` at the call site to stay type-safe without casts. Preserve this pattern when adding sortable list endpoints: add a new `CursorPayload` variant in `cursor.ts` with its own codec, and TypeScript's `never`-guarded switches in `encodeCursor`/`decodeCursor`/`sortConfigFor` plus the two switches in `repository.ts#list` will force you to handle it everywhere.
